@php
    // 6.2: enlace directo a WhatsApp Ventas. Sin número configurado se cae al formulario.
    $number = preg_replace('/\D/', '', (string) config('icce.whatsapp_number'));
    $href = $number
        ? 'https://wa.me/'.$number.'?text='.rawurlencode('Hola, me interesa cotizar con ICCE.')
        : '/contacto';
@endphp

<a href="{{ $href }}"
   @if($number) target="_blank" rel="noopener noreferrer" @endif
   class="group fixed bottom-5 right-5 z-40 flex items-center gap-3 rounded-full bg-[#25D366] py-3 pl-3 pr-4 text-carbon-950 shadow-lg shadow-carbon-950/25 transition hover:shadow-xl hover:shadow-carbon-950/30 sm:bottom-8 sm:right-8"
   aria-label="Escribir a ventas por WhatsApp">
    ...
    <span class="hidden text-sm font-bold sm:block">WhatsApp Ventas</span>
</a>
